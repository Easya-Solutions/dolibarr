...
class User extends CommonObject
{
	...
	public function update($user, $notrigger = 0, $nosyncmember = 0, $nosyncmemberpass = 0, $nosynccontact = 0)
	{
		...
		$this->setUpperOrLowerCase();
		$this->accountancy_code_user_general = trim($this->accountancy_code_user_general);
	...
	public function getNomUrl($withpictoimg = 0, $option = '', $infologin = 0, $notooltip = 0, $maxlen = 24, $hidethirdpartylogo = 0, $mode = '', $morecss = '', $save_lastsearch_value = -1)
	{
		...
		if (!empty($this->accountancy_code_user_general) || $option == 'accountancy') {
			$label .= '<br><b>'.$langs->trans("UserAccountancyCodeGenera").'</b>: '.$this->accountancy_code_user_general;
		}
		if (!empty($this->accountancy_code) || $option == 'accountancy') {
			$label .= '<br><b>'.$langs->trans("UserAccountancyCode").'</b>: '.$this->accountancy_code;
		}
		$company = '';
