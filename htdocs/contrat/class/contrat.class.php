...
class Contrat extends CommonObject
{
	...
	public function getListOfContracts($option = 'all', $status = [], $product_categories = [], $line_status = [])
	{
		...
		if (!empty($product_categories)) {
			...
			$sql .= " INNER JOIN ".MAIN_DB_PREFIX."categorie_product as cp ON cp.fk_product = cd.fk_product AND cp.fk_categorie IN (".$this->db->escape(implode(", ", $product_categories)).")";
		}
		$sql .= " WHERE c.fk_soc =".((int) $this->socid);
		$sql .= ($option == 'others') ? " AND c.rowid <> ".((int) $this->id) : "";
		$sql .= (!empty($status)) ? " AND c.statut IN (".$this->db->escape(implode(", ", $status)).")" : "";
		$sql .= (!empty($line_status)) ? " AND cd.statut IN (".$this->db->escape(implode(", ", $line_status)).")" : "";
